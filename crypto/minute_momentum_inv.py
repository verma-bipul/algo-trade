"""
BTC 1-Min Inverse Momentum (Mean Reversion)

Every minute:
- Red candle (close < open) -> BUY (bet on bounce)
- Green candle -> skip
- Hold 1 min, sell, repeat
"""
...
logger = get_logger("minute_momentum_inv")
tracker = PortfolioTracker("minute_momentum_inv", "BTC 1-Min Inverse", symbol=SYMBOL, initial_cash=BUDGET)
...
def run():
    logger.info("=== 1-Minute INVERSE Momentum Strategy Starting ===")
    ...
    while True:
        try:
            ...
            if candle is None:
                ...
                # Step 3: If RED and not holding, buy (inverse of momentum)
                if not is_green:
                    ...
                    if qty > 0 and tracker.can_buy(SYMBOL, qty, price):
                        logger.info(f"RED signal — buying {qty:.8f} BTC (mean reversion)")
                    ...
                    logger.info("GREEN signal — skipping")
